...
private:
	...
	int  PageRead(VIR_MEM_NODE_t *node, u8 *dst, u32 srcAddr, u32 len);
public:
	...
	int  DumpVirMemNodeContent_ToFile(const char *fileName, bool memDumpMode = false, bool dumpAllData = false);
};
...
public:
	...
	int  AddPageAttributes(u32 addr, u32 flags);
	int  RemovePageAttributes(u32 addr, u32 flags);
	int  ClearByteWrittenFlag(u32 startAddr, u32 len);
	int  DumpProtectedMemNodeContent_ToFile(const char *fileName1, const char *fileName2, bool memDumpMode = false, bool dumpAllData = false);
};
