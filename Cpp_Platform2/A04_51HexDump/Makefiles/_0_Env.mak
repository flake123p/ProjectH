...
SRC_DIR = src
LIB_MGR_DIR = ../_Library/lib_mgr
ifeq ($(OS),Windows_NT)
LIB_DIR = ../_Library/lib_win
LIB += -lpdcurses
else
...
# For g++
INC = -Iinclude -I../_Library/include
...
# .o檔的放置資料夾
build_dir = build
lib_build_dir = ../_Library/build
# 把所有的 .o檔 加上 build output資料夾的路徑
OBJS = $(patsubst %,$(build_dir)/%,$(obj_all))
OBJS += $(patsubst %,$(lib_build_dir)/%,$(obj_lib_mgr))
OBJS += $(patsubst %,$(lib_build_dir)/%,$(obj_lib))
LIB_OBJ_WITH_PATH = $(patsubst %,$(lib_build_dir)/%,$(obj_lib_mgr))
LIB_OBJ_WITH_PATH += $(patsubst %,$(lib_build_dir)/%,$(obj_lib))
